...
function TrasyMake() {
	...
	TrasyClass.prototype.trasaWstawDoBazy = function (opis, kiedy, kto) { // opis bez polskich znakow koniecznie
		var that = this;
	...
	//Trasa.prototype.dodaj = function (firma) {
	Trasa.prototype.zapiszPunkt = function (frmId, kolejnosc) {
		var that = this;
		var kolejnosc = kolejnosc || this.kolejnosc;
		var firma = impet.firmy[frmId];
		var quer = ["kto=", this.zdWykonawcaId, "&opis=brak&kiedy=#", this.kiedy, "#&trasaId=", this.is, "&kolejnosc=", kolejnosc].join(',');
		$.post(serwer + "/ajax/trasyPunktInsert.php/?" + quer).done(
			function (data) {
				...
				that.push(new Punkt(ele, ind, that.opacity, false, that));
				kolejnosc++;
				that.show();
			}
		)
	...
	Trasa.prototype.dodaj = function (firma) {
		...
		var element = {
			firmaId: firma.id,
			id: punktNewId--,
			kto: impet.settings.lastUserId,
			kiedy: new Date(),
			trasaId: this.id,
			zakonczone: false
		};
		var punkt = new Punkt(element, this.kolejnosc++, that.opacity, that);
	...
	Trasa.prototype.listaHTML = function () {
		...
		this.forEach(function (element, index) {
			...
			result += '<li data-trasaid="' + element.id + '" data-id="' + element.firmaId + '" class=\"ui-widget-content\">&nbsp;' + element.kolejnosc +
				'. <b  class="nazwaNaLiscie" title=\"' + element.kiedy.toJSON().slice(0, 10) + '\">' + element.firma.nazwa +
