...
var serwer = 'http://192.168.2.220/';
//'http://localhost'; // document.location.origin;
...
impet._ustawieniaWczytaj = function() {
    $('input[store]').each(function(ind, ele) {
        // każdy node posiadający atrybut store i bedacy inputem będzie zapamiętywany
        if (ele.type === 'checkbox') {
            ...
            // nody mogą zawierać wartośc domyślną
            $('#val' + ele.id).html(ele.value);
        // wszystkie nody rozpoczynające sie od val + id inputa będą mialy jego wartosc
        }
    }
    );
}
...
function _Firma(record) {
    ...
    opt.clickable = false;
    this.markerNazwa = new Markero(opt);
    //google.maps.
...
;
/////////////////////////////////////////////////////////////////////////////////////////////////////////
Object.defineProperty(impet, 'fb', {
    ////////////////////////////////////    FB
    get: function() {
...
function createFormForCompany() {
	...
	'<p style="padding: 15px;"><button onclick="window.firmyEdycja.dialog(\'close\');" id="anulujEdycje">Anuluj</button> ' + 
	'<button onclick="impet.fzapiszEdycje()" id="zapiszEdycje">Zapisz</button></div></p>';
	formularzFirma+= "<div id='codeAds'>&nbsp;</div>";
	...
	window.firmyEdycja.getFB = function() {
		...
		var address2=(impet.miejscowoscId[impet.fb.miejscowoscId].nazwa + " " + impet.fb.ulica);
		codeAd(address2,($('#codeAds').html).bind($('#codeAds')));
	}
	...
	function fzapiszEdycje() {
		...
		$.get(serwer + '/ajax/ajaxuniversal.php', plain).done(function(){
            impet.fb.setPosition(new google.maps.LatLng($('#wspN').val(),$('#wspE').val()));
			window.firmyEdycja.dialog('close');
		});
	}
...
google.maps.event.addDomListener(window, 'load', impet._mapCreate);
///////////////// tu ustawienie punktu inicjalizacji
